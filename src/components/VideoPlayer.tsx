...
// ✅ Added useState to handle our custom menu logic
import { type CSSProperties, type ComponentProps, forwardRef, type ReactNode, isValidElement, useState } from 'react';
...
// ✅ FOOLPROOF CUSTOM PLAYBACK RATE MENU
function PlaybackRateMenuItems(): ReactNode {
  ...
  const handleRateChange = (newRate: string) => {
    setRate(newRate);
    // Directly target the HTML5 video element to change speed instantly
  ...
  return (
    <Menu.RadioGroup className="media-menu__group" value={rate} onValueChange={handleRateChange} aria-label="Playback rate">
      {options.map((option) => (
        <Menu.RadioItem key={option.value} className="media-menu__item" value={option.value}>
          <span>{option.label}</span>
          <Menu.ItemIndicator checked={option.value === rate} forceMount className="media-menu__indicator">
...
export function VideoPlayer({ src, className, poster, subtitles, ...rest }: VideoPlayerProps): ReactNode {
  return (
    <Player.Provider>
        ...
        <Controls.Root className="media-surface media-controls">
          <Tooltip.Provider>
            ...
            <div className="media-button-group">
              ...
              {/* ✅ FIXED PLAYBACK RATE MENU UI */}
              <Menu.Root side="top" align="center">
                <Menu.Trigger render={<Button className="media-button--playback-rate" />}>
                  <span style={{ fontSize: '0.85rem', fontWeight: 'bold' }}>Speed</span>
                </Menu.Trigger>
...
// ================================================================
// Components & Utilities (unchanged)
// ================================================================
...
function isRenderProp(value: unknown): value is RenderProp<unknown> { return typeof value === 'function' || isValidElement(value); }